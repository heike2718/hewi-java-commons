...
import de.egladil.web.commons_officetools.ZipParser;
...
public class ODSParser {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ODSParser.class);
	 ...
	 * @throws IOException
	 */
	public String getContentSafe(final InputStream in, final int maxLengthExtracted) throws IOException, OfficeToolsSecurityException {
	...
	void checkVulnerableXml(final String xml) throws IOException {
		...
		try (InputStream in = new ByteArrayInputStream(xml.getBytes())) {
			...
			throw new OfficeToolsSecurityException("XML enthält DOCTYPE");
		}
	}
}
